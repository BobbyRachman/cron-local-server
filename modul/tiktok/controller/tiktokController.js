...
var tiktokCronLog = require('../model/tiktokCron');
...
module.exports = cron.schedule('*/5 * * * *',async ()=>{
  try {
    ...
    let shops = await Marketplace.aggregate([{$lookup:{from: "categorieMp",localField: "fk_brand",foreignField: "rowid",as: "detail_brand"}}, {$match:{'fk_channel' : '29','sts':'1'}}]);
    let marketplaceId = "tiktok";
    for(let shop of shops){
      try {
        ...
        let checkCron = await tiktokCronLog.find({ $and : [{"shopid" : shopid}, {"start_date" : { $ne : null }}] }, {"start_date": 1,"end_date": 1}).sort({"start_date": -1});
        if(checkCron.length === 0){
          ...
          await tiktokCronLog.create(dataCron);
        }
        ...
        if(diff >= 24){
          // console.log("ALG 2")
          let queryCron = await tiktokCronLog.find({ $and : [{"shopid" : shopid}, {"start_date" : { $ne : null }}] }, {"start_date": 1,"end_date": 1,"n_transaction" : 1}).sort({"start_date": -1});
        ...
        await tiktokCronLog.create(cronLog);
