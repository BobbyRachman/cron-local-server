...
var jdidCronLog = require('../model/jdidCron');
...
module.exports = cron.schedule('*/5 * * * *',async ()=>{
  try {
    console.log("Jdid Start");
    // Get Bukalapak's Shops from DB
    let shops = await Marketplace.aggregate([{$lookup:{from: "categorieMp",localField: "fk_brand",foreignField: "rowid",as: "detail_brand"}}, {$match:{'fk_channel' : '15','sts':'1'}}]);
    let marketplaceId = "jdid";
    for(let shop of shops){
      try {
        ...
        let checkCron = await jdidCronLog.find({ $and : [{"shopid" : shopid}, {"start_date" : { $ne : null }}] }, {"start_date": 1,"end_date": 1}).sort({"start_date": -1});
        if(checkCron.length === 0){
          ...
          await jdidCronLog.create(dataCron);
        }
        ...
        if(diff >= 24){
          // console.log("ALG 2")
          let queryCron = await jdidCronLog.find({ $and : [{"shopid" : shopid}, {"start_date" : { $ne : null }}] }, {"start_date": 1,"end_date": 1,"n_transaction" : 1}).sort({"start_date": -1});
        ...
        await jdidCronLog.create(cronLog);
