  ...
  try {
    ...
    for(let shop of shops){
      try {
        ...
        let checkCron = await zaloraCronLog.find({ $and : [{"shopid" : shopid}, {"start_date" : { $ne : null }}] }, {"start_date": 1,"end_date": 1}).sort({"start_date": -1});
        if(checkCron.length === 0){
          ...
          await zaloraCronLog.create(dataCron);
        }
        ...
        if(diff >= 24){
          // console.log("ALG 2")
          let queryCron = await zaloraCronLog.find({ $and : [{"shopid" : shopid}, {"start_date" : { $ne : null }}] }, {"start_date": 1,"end_date": 1,"n_transaction" : 1}).sort({"start_date": -1});
        ...
        await zaloraCronLog.create(cronLog);
