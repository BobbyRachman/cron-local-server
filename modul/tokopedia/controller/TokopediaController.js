...
var tokopediaCronLog = require('../model/tokopediaCron');
...
module.exports = cron.schedule('*/5 * * * *',async ()=>{
  try {
    console.log("Tokopedia Start");
    // Get Bukalapak's Shops from DB
    let shops = await Marketplace.aggregate([{$lookup:{from: "categorieMp",localField: "fk_brand",foreignField: "rowid",as: "detail_brand"}}, {$match:{'fk_channel' : '12','sts':'1'}}]);
    let marketplaceId = "tokopedia";
    for(let shop of shops){
      try {
        ...
        let checkCron = await tokopediaCronLog.find({ $and : [{"shopid" : shopid}, {"start_date" : { $ne : null }}] }, {"start_date": 1,"end_date": 1}).sort({"start_date": -1});
        if(checkCron.length === 0){
          ...
          await tokopediaCronLog.create(dataCron);
        }
        ...
        if(diff >= 24){
          // console.log("ALG 2")
          let queryCron = await tokopediaCronLog.find({ $and : [{"shopid" : shopid}, {"start_date" : { $ne : null }}] }, {"start_date": 1,"end_date": 1,"n_transaction" : 1}).sort({"start_date": -1});
        ...
        await tokopediaCronLog.create(cronLog);
