...
module.exports = cron.schedule('*/5 * * * *',async ()=>{
  try {
    ...
    let shops = await Marketplace.aggregate([{$lookup:{from: "categorieMp",localField: "fk_brand",foreignField: "rowid",as: "detail_brand"}}, {$match:{'fk_channel' : '13','sts':'1'}}]);
    ...
    for(let shop of shops){
      try {
        ...
        let checkCron = await BukalapakCronLog.find({ $and : [{"shopid" : shopid}, {"start_date" : { $ne : null }}] }, {"start_date": 1,"end_date": 1}).sort({"start_date": -1});
        if(checkCron.length === 0){
          ...
          await BukalapakCronLog.create(dataCron);
        }
        ...
        if(salesOrder.length > 0){
          ...
          sd = new Date("2022-08-11T15:00:00.000Z");
        ...
        if(diff >= 24){
          // console.log("ALG 2")
          let queryCron = await BukalapakCronLog.find({ $and : [{"shopid" : shopid}, {"start_date" : { $ne : null }}] }, {"start_date": 1,"end_date": 1,"n_transaction" : 1}).sort({"start_date": -1});
        ...
        await BukalapakCronLog.create(cronLog);
