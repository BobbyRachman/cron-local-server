  ...
  try {
    ...
    for(let shop of shops){
      try {
        ...
        let checkCron = await lazadaCronLog.find({ $and : [{"shopid" : shopid}, {"start_date" : { $ne : null }}] }, {"start_date": 1,"end_date": 1}).sort({"start_date": -1});
        if(checkCron.length === 0){
          ...
          await lazadaCronLog.create(dataCron);
        }
        ...
        if(diff >= 24){
          // console.log("ALG 2")
          let queryCron = await lazadaCronLog.find({ $and : [{"shopid" : shopid}, {"start_date" : { $ne : null }}] }, {"start_date": 1,"end_date": 1,"n_transaction" : 1}).sort({"start_date": -1});
        ...
        await lazadaCronLog.create(cronLog);
